...
run = do
  input <- getContents
  let vs = toVec input
  print $ Vector.head $ eval 0 $ vs // [(1, 12), (2, 2)]
  print $ (\(noun, verb) -> 100 * noun + verb) $ findInputPair 19690720 100 vs
...
eval' :: Int -> Vector Int -> ComputerState
eval' ip vs =
    if ip < Vector.length vs then
        case vs ! ip of
          1 ->
...
evalSafe :: [(Int, Vector Int)] -> Int -> Vector Int -> Maybe (Vector Int)
evalSafe acc ip vs =
    if (ip, vs) `elem` acc then
        Nothing
    else case eval' ip vs of
           Running (ip', vs') -> evalSafe ((ip, vs) : acc) ip' vs'
           Terminated vs' -> Just vs'
           Errored err -> Nothing

findInputPair :: Int -> Int -> Vector Int -> (Int, Int)
findInputPair expectedOutput threshold vs =
    let tryInput noun verb =
            case evalSafe [] 0 $ vs // [(1, noun), (2, verb)] of
              Just vs' -> Just $ Vector.head vs'
              Nothing -> Nothing
        helper noun verb | verb > threshold = error "Solution not found"
                         | noun > threshold = helper 0 (verb + 1)
                         | otherwise = if tryInput noun verb == Just expectedOutput then
                                           (noun, verb)
                                       else
                                           helper (noun + 1) verb
    in helper 0 0
